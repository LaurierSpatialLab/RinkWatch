define(
	[
		...
		return declare("modules/ClusterLayer", [GraphicsLayer], {
			...
			onClick: function(e) {
				...
				// If queryOnClick is set to false, this is an indication that something else, like drawing, is happening, so don't display the infoWindow
				if (!rinkwatch.queryOnClick) return;
				...
				for ( var i = 0, il = this._clusterData.length; i < il; i++) {
					if ( e.graphic.attributes.clusterId == this._clusterData[i].attributes.clusterId ) {
						// Add last skateable readings to popup
						var rinkid = this._clusterData[i].attributes.id;

						if (rinkwatch.readingArray[rinkid]) {
							this._clusterData[i].attributes.reading = [];
							// Add the readingArray reading data to each rink for the infoTemplate
							if (rinkwatch.readingArray[rinkid].length > 4) {
								var curRink = 0;
								array.some(rinkwatch.readingArray[rinkid], function(r) {
									this._clusterData[i].attributes.reading.push(r.attributes);
									curRink++;
									if (curRink > 4) {
										return true;
									}
								...
								this._clusterData[i].attributes.reading.push(rinkwatch.readingArray[rinkid][0].attributes);
							}
						}
						// if (rinkwatch.readingArray[rinkid]) {
						// 	this._clusterData[i].attributes.reading = rinkwatch.readingArray[rinkid][0].attributes.reading_data;
						// }
						singles.push(this._clusterData[i]);
					}
				}
				if ( singles.length > this._maxSingles ) {
					alert("Sorry, that cluster contains more than " + this._maxSingles + " points. Please zoom in for more detail.");
					return;
				} else {
					// stop the click from bubbling to the map
					e.stopPropagation();
					// Set Info Window to a larger size
					this._map.infoWindow.resize(300,300);
			...
			_updateClusterGeometry: function(c) {
				...
				var cg = array.filter(this.graphics, function(g) {
					return ! g.symbol &&
